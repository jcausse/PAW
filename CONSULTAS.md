...
1. Esta bien haber sacado `<link>` de los POM?
2. Esta bien `<httpConnector>` para envolver `<port>` en la dependencia de Jetty en el POM de `webapp`?
    ```xml
    <plugin>
   ...
   ```
3. Revisar los warnings de este estilo al usar `make run`:
   ```text
   ...
   ```
4. Es necesario `language="java"` en los JSP?
